...
def SKIP_test_parse_grammar() -> None:
    grammar_source = """
    start: sum NEWLINE
    sum: t1=term '+' t2=term { action } | term
    term: NUMBER
...
def SKIP_test_parse_grammar_with_types() -> None:
    grammar = """
...
def SKIP_test_long_rule_str() -> None:
    grammar_source = """
...
def SKIP_test_typed_rules() -> None:
    grammar = """
...
def SKIP_test_expr_grammar() -> None:
    grammar = """
    start: sum NEWLINE
    sum: term '+' term | term
    term: NUMBER
...
def SKIP_test_optional_operator() -> None:
    grammar = """
    start: sum NEWLINE
    sum: term ('+' term)?
    term: NUMBER
...
def SKIP_test_optional_literal() -> None:
    grammar = """
    start: sum NEWLINE
    sum: term '+' ?
    term: NUMBER
...
def SKIP_test_alt_optional_operator() -> None:
    grammar = """
    start: sum NEWLINE
    sum: term ['+' term]
    term: NUMBER
...
def SKIP_test_repeat_0_simple() -> None:
    grammar = """
...
def SKIP_test_repeat_0_complex() -> None:
    grammar = """
...
def SKIP_test_repeat_1_simple() -> None:
    grammar = """
    start: thing thing+ NEWLINE
    thing: NUMBER
    """
...
def SKIP_test_repeat_1_complex() -> None:
    grammar = """
    start: term ('+' term)+ NEWLINE
    term: NUMBER
    """
...
def SKIP_test_repeat_with_sep_simple() -> None:
    grammar = """
    start: ','.thing+ NEWLINE
    thing: NUMBER
    """
...
def SKIP_test_left_recursive() -> None:
    grammar_source = """
    start: expr NEWLINE
...
def SKIP_test_python_expr() -> None:
    grammar = """
...
def SKIP_test_advanced_left_recursive() -> None:
    grammar_source = """
...
def SKIP_test_mutually_left_recursive() -> None:
    grammar_source = """
    start: foo 'E'
    foo: bar 'A' | 'B'
    bar: foo 'C' | 'D'
...
def SKIP_test_nasty_mutually_left_recursive() -> None:
...
def SKIP_test_named_lookahead_error() -> None:
    grammar = """
    start: foo=!'x' NAME
    """
    with pytest.raises(SyntaxError):
...
def SKIP_test_start_leader() -> None:
    grammar = """
    start: attr | NAME
    attr: start '.' NAME
    """
...
def SKIP_test_left_recursion_too_complex() -> None:
    grammar = """
    start: foo
    foo: bar '+' | baz '+' | '+'
    bar: baz '-' | foo '-' | '-'
...
def SKIP_test_cut() -> None:
    ...
    parser_class = make_parser(grammar)
    verbose_=False
    node = parse_string("(1)", parser_class, verbose=verbose_)
    assert node == [
...
def SKIP_test_cut_early_exit() -> None:
    grammar = """
    start: '(' ~ expr ')' | '(' name ')'
    expr: NUMBER
    name: NAME
    ...
    verbose_=False
    with pytest.raises(SyntaxError):
        parse_string("(a)", parser_class, verbose=verbose_)


def SKIP_test_dangling_reference() -> None:
    grammar = """
    start: foo ENDMARKER
    foo: bar NAME
    """
...
def SKIP_test_bad_token_reference() -> None:
    grammar = """
    start: foo
    foo: NAMEE
    """
...
def SKIP_test_missing_start() -> None:
    grammar = """
    foo: NAME
    """
    with pytest.raises(GrammarError):
...
def SKIP_test_soft_keyword() -> None:
    grammar = """
    start:
    ...
    parser_class = make_parser(grammar)
    verbose_=False
    assert parse_string("number 1", parser_class, verbose=verbose_) == 1
    assert parse_string("string 'b'", parser_class, verbose=verbose_) == "'b'"
    assert parse_string("number test 1", parser_class, verbose=verbose_) == "test = 1"
    assert parse_string("string test 'b'", parser_class, verbose=verbose_) == "test = 'b'"
    with pytest.raises(SyntaxError):
        parse_string("test 1", parser_class, verbose=verbose_)


def SKIP_test_forced() -> None:
    grammar = """
    start: NAME &&':' | NAME
    """
    parser_class = make_parser(grammar)
    verbose_=False
    assert parse_string("number :", parser_class, verbose=verbose_)
...
def SKIP_test_forced_with_group() -> None:
    grammar = """
    start: NAME &&(':' | ';') | NAME
    """
    parser_class = make_parser(grammar)
...
def SKIP_test_unreachable_explicit() -> None:
    source = """
...
def SKIP_test_unreachable_implicit1() -> None:
    source = """
...
def SKIP_test_unreachable_implicit2() -> None:
    source = """
    start: NAME | '(' invalid_input ')'
    invalid_input: NUMBER { None }
    """
...
def SKIP_test_unreachable_implicit3() -> None:
    source = """
...
def SKIP_test_locations_in_alt_action_and_group() -> None:
    grammar = """
